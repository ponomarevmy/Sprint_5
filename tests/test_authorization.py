...
class Test_Authorization:
    def test_authorization_from_log_in_account(self, driver):
        WebDriverWait(driver, 5).until(
            EC.element_to_be_clickable(Locators.BTN_ENTER_ACCOUNT))
        driver.find_element(*Locators.BTN_ENTER_ACCOUNT).click()
        WebDriverWait(driver, 5).until(
            EC.element_to_be_clickable(Locators.BTN_ENTER))
        driver.find_element(
            *Locators.INPUT_EMAIL).send_keys(data.User.login)
        driver.find_element(
            *Locators.INPUT_PASSWORD).send_keys(data.User.password)
    ...
    def test_authorization_in_registration_form(self, driver):
        ...
        WebDriverWait(driver, 5).until(
            EC.element_to_be_clickable(Locators.REF_ENTER_FROM_SECTION))
    ...
    def test_authorization_password_recovery_form(self, driver):
        ...
        WebDriverWait(driver, 5).until(
            EC.element_to_be_clickable(Locators.ENTER_FROM_PASSWORD_RESTORE))
